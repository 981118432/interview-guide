...
DOC_LABELS_ZH = {
    ...
    "01-inference-fundamentals": "推理基础",
    "02-kv-cache-and-context-caching": "KV Cache 与上下文缓存",
    "03-speculative-decoding": "投机解码",
    "04-batching-strategies": "批处理策略",
    "05-paged-attention": "PagedAttention",
    "06-serving-infrastructure": "推理服务基础设施",
    "07-cost-optimization-playbook": "成本优化实战",
    "08-diffusion-llms": "Diffusion LLM",
    "09-on-device-and-edge-deployment": "端侧与边缘部署",
    "01-prompt-engineering-fundamentals": "Prompt 工程基础",
    "02-few-shot-and-icl": "Few-shot 与 ICL",
    "03-chain-of-thought": "Chain-of-Thought",
    "04-tree-of-thought": "Tree-of-Thought",
    "05-context-engineering": "上下文工程",
    "06-structured-generation": "结构化生成",
    "07-prompt-optimization-dspy": "Prompt 优化与 DSPy",
    "08-prompt-injection-defense": "Prompt 注入防御",
    "01-rag-fundamentals": "RAG 基础",
    "02-chunking-strategies": "分块策略",
    "03-embedding-models": "Embedding 模型",
    "04-vector-databases": "向量数据库",
    "05-hybrid-search": "混合搜索",
    "06-reranking-strategies": "重排序策略",
    "07-graph-rag": "GraphRAG",
    "08-agentic-rag": "Agentic RAG",
    "09-advanced-retrieval-patterns": "高级检索模式",
    "10-contextual-retrieval": "上下文检索",
    "11-late-interaction-colbert": "Late Interaction 与 ColBERT",
    "12-multimodal-rag": "多模态 RAG",
    "13-rag-evaluation-patterns": "RAG 评估模式",
    "14-production-rag-at-scale": "生产级 RAG",
    "15-data-engineering-for-ai": "面向 AI 的数据工程",
    "01-agent-fundamentals": "Agent 基础",
    "02-reasoning-loops-react-and-beyond": "推理循环：ReAct 及后续",
    "03-tool-use-and-mcp": "工具使用与 MCP",
    "04-multi-agent-orchestration": "多 Agent 编排",
    "05-agent-memory-and-state": "Agent 记忆与状态",
    "06-planning-and-decomposition": "规划与分解",
    "07-error-handling-and-recovery": "错误处理与恢复",
    "08-human-in-the-loop-patterns": "人在环模式",
    "09-agentic-security-and-sandboxing": "Agent 安全与沙箱",
    "10-evaluating-agentic-systems": "Agent 系统评估",
    "11-durable-execution": "持久执行",
    "12-loop-engineering": "循环工程",
    "README": "阅读地图",
}
...
<head>
  ...
  <style>
    ...
    * { box-sizing:border-box; }
    ...
    aside { position:fixed; inset:0 auto 0 0; width:280px; overflow-y:auto; padding:24px 16px; background:#111827; color:#dbe5f2; }
    ...
    .section { border:1px solid #263650; border-radius:8px; overflow:hidden; }
    .section summary { padding:9px 10px; color:#dbe5f2; background:#18243a; cursor:pointer; font-size:12px; font-weight:800; list-style:none; }
    .section summary::-webkit-details-marker { display:none; }
    .section summary::before { display:inline-block; width:16px; color:#8fa5bf; content:'▶'; font-size:9px; transition:transform .15s ease; }
    .section[open] summary::before { transform:rotate(90deg); }
    .section-docs { display:grid; gap:3px; }
    .section-docs { padding:5px; background:#111827; }
    ...
    main { min-width:0; margin-left:280px; padding:32px clamp(18px,4vw,56px); }
    ...
    @media (max-width:760px) { aside { position:static; width:auto; max-height:none; padding:16px; } .doc-list { display:grid; } main { margin-left:0; padding:24px 16px; } h2 { font-size:24px; } }
  </style>
</head>
<body>
<div class="app">
...
function render() {
  ...
  if (!openSections.size) sections.forEach(section => openSections.add(section.id));
  document.getElementById('doc-list').innerHTML = sections.map(section => `<details class="section" data-section="${section.id}" ${openSections.has(section.id) ? 'open' : ''}><summary>${section.label}</summary><div class="section-docs">${section.items.map(({item, index}) => `<button class="${index === current ? 'active' : ''}" data-doc="${index}"><span class="doc-name">${item.label}</span><span class="doc-file">${item.filename}</span></button>`).join('')}</div></details>`).join('');
  document.querySelectorAll('[data-section]').forEach(section => section.addEventListener('toggle', () => { if (section.open) openSections.add(section.dataset.section); else openSections.delete(section.dataset.section); }));
